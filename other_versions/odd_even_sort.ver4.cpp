...
void Worker::sort()
{
  ...
  if (block_size < 2048)
  {
    int main_rank = proc_size / 2;
    int left_size = block_size * l_proc_size;
    int right_size = n - left_size - block_len;
    float *sort_arr;
    if (rank == main_rank)
    {
      if (l_proc_size)
      {
        if (r_proc_size)
        {
          ...
          sort_arr = new float[n];
          float *left_pos = sort_arr;
          float *my_pos = sort_arr + left_size;
          float *right_pos = sort_arr + left_size + block_len;
          MPI_Irecv(left_pos, left_size, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD, &requests[0]);
          MPI_Irecv(right_pos, right_size, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD, &requests[1]);
          memcpy(my_pos, data, block_len * sizeof(float));
          MPI_Waitall(2, requests, MPI_STATUSES_IGNORE);
          std::sort(sort_arr, sort_arr + n);
          MPI_Isend(left_pos, left_size, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD, &requests[0]);
          MPI_Isend(right_pos, right_size, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD, &requests[1]);
          memcpy(data, my_pos, block_len * sizeof(float));
          MPI_Waitall(2, requests, MPI_STATUSES_IGNORE);
        }
        else
        {
          MPI_Request request;
          sort_arr = new float[left_size + block_len];
          float *left_pos = sort_arr;
          float *my_pos = sort_arr + left_size;
          MPI_Irecv(left_pos, left_size, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD, &request);
          memcpy(my_pos, data, block_len * sizeof(float));
          MPI_Wait(&request, MPI_STATUS_IGNORE);
          std::sort(sort_arr, sort_arr + n);
          MPI_Isend(left_pos, left_size, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD, &request);
          memcpy(data, my_pos, block_len * sizeof(float));
          MPI_Wait(&request, MPI_STATUS_IGNORE);
        }
      }
    ...
    else if (rank < main_rank)
    {
      if (l_proc_size)
      {
        ...
        memcpy(my_pos, data, block_len * sizeof(float));
        MPI_Wait(&request, MPI_STATUS_IGNORE);
        MPI_Send(sort_arr, left_size + block_len, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD);
        MPI_Recv(sort_arr, left_size + block_len, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Isend(left_pos, left_size, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD, &request);
        memcpy(data, my_pos, block_len * sizeof(float));
        MPI_Wait(&request, MPI_STATUS_IGNORE);
      }
      ...
      {
        MPI_Send(data, block_len, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD);
        MPI_Recv(data, block_len, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      }
    }
    else if (rank > main_rank)
    {
      ...
      {
        MPI_Request request;
        sort_arr = new float[block_len + right_size];
        float *right_pos = sort_arr + block_len;
        float *my_pos = sort_arr;
        MPI_Irecv(right_pos, right_size, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD, &request);
        memcpy(my_pos, data, block_len * sizeof(float));
        MPI_Wait(&request, MPI_STATUS_IGNORE);
        MPI_Send(sort_arr, right_size + block_len, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD);
        MPI_Recv(sort_arr, right_size + block_len, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Isend(right_pos, right_size, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD, &request);
        memcpy(data, my_pos, block_len * sizeof(float));
        MPI_Wait(&request, MPI_STATUS_IGNORE);
      }
      else
  ...
  else
  {
    ...
    for (int swap = 0, stage = 0; swap < max_swap; stage = (++swap) % 2)
    {
      ...
      if (swap == check_point)
      {
        ...
        {
          if (l_proc_size) // 多于一个进程才发信息
          {
            if (!r_proc_size) // 两个进程：mi == 1
            {
