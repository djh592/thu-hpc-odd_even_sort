...
void Worker::sort()
{
  ...
  // 交换数据
  int req_rank = 0;
  MPI_Request *req_list = new MPI_Request[3 * max_swap]; // send_list：用于统一回收 Isend
  for (int swap = 0, stage = 0; swap < max_swap; stage = (++swap) % 2)
  {
    if (parity == stage) // 接收进程（右侧交换）
    {
      if (r_proc_size)
      {
        ...
        float right_min;
        MPI_Isend(&my_max, 1, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD, &req_list[req_rank++]);
        MPI_Recv(&right_min, 1, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        if (my_max > right_min) // 需要交换
        {
          int recv_len;
          MPI_Irecv(&recv_len, 1, MPI_INT, rank + 1, 0, MPI_COMM_WORLD, &req_list[req_rank]); //
          float *beg_pos = std::upper_bound(sort_arr, sort_arr + block_len, right_min);
          MPI_Wait(&req_list[req_rank], MPI_STATUS_IGNORE);
          ...
          MPI_Isend(&sort_arr[block_len], recv_len, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD, &req_list[req_rank++]);
        }
      }
    }
    else // 发送进程（左侧交换）
    {
      if (l_proc_size)
      {
        ...
        float left_max;
        MPI_Isend(&my_min, 1, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD, &req_list[req_rank++]);
        ...
        if (my_min < left_max) // 需要交换
        {
          ...
          MPI_Isend(&send_len, 1, MPI_INT, rank - 1, 0, MPI_COMM_WORLD, &req_list[req_rank++]);
          MPI_Isend(sort_arr, send_len, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD, &req_list[req_rank++]);
